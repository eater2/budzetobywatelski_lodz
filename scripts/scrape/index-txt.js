...
async function main() {
  console.log('=== Łódź Civic Budget Scraper (from TXT) ===\n');
  ...
  const inputFile = process.argv.find(arg => arg.endsWith('.txt')) || 
                    path.join(process.cwd(), 'data', 'project-urls.txt');
  
  try {
    // Step 1: Load and scrape projects from TXT file
    console.log(`Step 1: Loading project URLs from ${inputFile}...\n`);
    const scraper = new BudgetScraperFromTxt(options);
    
    // Load URLs from file
    await scraper.loadProjectsFromFile(inputFile);
